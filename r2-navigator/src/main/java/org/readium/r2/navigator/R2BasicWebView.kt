...
open class R2BasicWebView(context: Context, attrs: AttributeSet) : WebView(context, attrs) {
    ...
    private var callbackClickExternalLink : ((Uri) -> Unit)? = null
...
//            openExternalLink(request.url)
            callbackClickExternalLink?.invoke(request.url)
            return true
        }
    ...
    fun  setCallbackClickExternalLink(callbackClickExternalLink : ((Uri) -> Unit)){
        this.callbackClickExternalLink = callbackClickExternalLink
    }
