...
package com.newit.school_guide.feat.book
...
import com.newit.school_guide.feat.book.utils.extensions.copyToTempFile
import org.readium.r2.testapp.utils.extensions.copyToTempFile
import com.newit.school_guide.feat.book.utils.extensions.moveTo
...
class BookshelfViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private var server: Server = BaseApplication.server
    ...
    private var r2Directory: String = BaseApplication.R2DIRECTORY
...
//    val showProgressBar = ObservableBoolean()
    ...
    fun deleteBook(book: Book) = viewModelScope.launch {
        ...
        tryOrNull { File("${BaseApplication.R2DIRECTORY}covers/${book.id}.png").delete() }
    }
    ...
    private suspend fun addPublicationToDatabase(
        ...
        preferences.edit().putLong("id_book", id).apply()
        preferences.edit().putString("href_book", href).apply()
        storeCoverImage(publication, id.toString())
        return id
    }

    fun getBookHref(): String{
        return preferences.getString("href_book","")?:""
    }

    fun getBookId(): Long{
        return preferences.getLong("id_book",0)
    }

    fun copySamplesFromAssetsToStorage() = viewModelScope.launch(Dispatchers.IO) {
        withContext(Dispatchers.IO) {
            if (!preferences.contains("books")) {
                val dir = File(r2Directory)
                if (!dir.exists()) {
                    dir.mkdirs()
                }
                val samples = r2Application.assets.list("books")?.filterNotNull().orEmpty()
                for (element in samples) {
                    val file =
                        r2Application.assets.open("books/$element").copyToTempFile(r2Directory)
                    if (file != null)
                        importPublication(file)
                    else if (BuildConfig.DEBUG)
                        error("Unable to load sample into the library")
                }
                preferences.edit().putBoolean("books", true).apply()
            }
        }
    }
...
//                showProgressBar.set(false)
    ...
    fun openBook(
        bookHref: String, context: Context,
        callback: (file: FileAsset, mediaType: MediaType?, publication: Publication, remoteAsset: FileAsset?, url: URL?) -> Unit
    ) = viewModelScope.launch {

        val remoteAsset: FileAsset? =
            tryOrNull { URL(bookHref).copyToTempFile(r2Directory)?.let { FileAsset(it) } }
        val asset = remoteAsset // remote file
            ?: FileAsset(File(bookHref)) // local file

        streamer.open(asset, allowUserInteraction = true, sender = context)
            .onFailure {
                Timber.d(it)
                Toast.makeText(BaseApplication.getInstance(), "Book error", Toast.LENGTH_SHORT).show()
                channel.send(Event.OpenBookError(it.getUserMessage(r2Application)))
            }
            .onSuccess {
                if (it.isRestricted) {
